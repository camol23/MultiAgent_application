...
def create_envs_multirobot(gym, sim, robot, num_agents, num_envs, envs_per_row, agents_spacing, env_spacing):
    # load robot from urdf
    asset = load_asset(gym, sim, robot, fix_base_link)
    ...
    # set up the env grid
    # env_spacing = num_agents*agents_spacing
    env_lower = gymapi.Vec3(-env_spacing, -env_spacing, 0.0)
    env_upper = gymapi.Vec3(env_spacing, env_spacing, env_spacing)

    # cache some common handles for later use
    envs = []
    actor_handles = []                                                  # list of all envs
    actor_handle_env = []                                               # list by env 
    height = init_height

    # create and populate the environments
    for i in range(num_envs):
        env = gym.create_env(sim, env_lower, env_upper, envs_per_row) # Original
        envs.append(env)

        for j in range(num_agents):
            pose = gymapi.Transform()
            pose.p = gymapi.Vec3(0.0, j*agents_spacing, height)

            actor_handle = gym.create_actor(env, asset, pose, "MyActor_"+str(i), group=i, filter=1)
            actor_handle_env.append(actor_handle)

        actor_handles.append(actor_handle_env)
